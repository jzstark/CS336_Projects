...
from cs336_basics.learning import cross_entropy_loss, learning_rate_schedule, gradient_clipping, AdamW
...
def run_embedding(
    ...
    token_long : torch.LongTensor = token_ids.long()
    out = embedding_layer.forward(token_long)
    return out
...
def run_multihead_self_attention(
    ...
    multi_head_attention = MultiHeadAttention(
        d_model=d_model,
        num_heads=num_heads)
    multi_head_attention.update_weights(q_proj_weight, k_proj_weight, 
...
def run_rope(
    ...
    """
    rope_layer = RotaryPositionalEmbedding(d_k=d_k, theta=theta, 
                                           max_seq_len=max_seq_len)
    out = rope_layer(in_query_or_key, token_positions)
    assert out.shape == in_query_or_key.shape, \
        f"Expected output shape {in_query_or_key.shape}, got {out.shape}"
    return out
...
def run_rmsnorm(
    ...
    with torch.no_grad():
        rmsnorm_layer.gain.copy_(weights)
...
def get_adamw_cls() -> type[torch.optim.Optimizer]:
    ...
    """
    return AdamW #torch.optim.AdamW
